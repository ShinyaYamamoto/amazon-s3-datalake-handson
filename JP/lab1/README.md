...
CloudFormationによって作成されたEC2へログインできることを確認します。

1. AWS マネージメントコンソールのサービス一覧から**EC2**　を選択、画面左のペインから **インスタンス**　を選択します。

2. インスタンス名が、**handson-minilake** となっているインスタンスがCloudformationで作成されたEC2インスタンスです。

3. インスタンス名にマウスカーソルを合わせ、左クリック、 メニューから**[接続]** を選択します。

4. [インスタンスに接続]という画面で、**セッションマネージャ**を選択し、右下の**[接続]**をクリックします。

5.  新しいタブへ画面が切り替わり、ブラウザ上でコマンド実行できるシェルが起動します。
